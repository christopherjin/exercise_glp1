...
plot_rna_immuno_combined = function(rna_data,
  ...
  immuno_plot = ggplot(merged_data, aes(x = Tissue, y = Average.y, fill = Tissue)) +
    ...
    geom_text(
      ...
      aes(x = Tissue, y = 5, label = "Level not detected."),
      inherit.aes = FALSE,
      size = 2.5,
      angle = 90,
      vjust = -0.3
    ...
    geom_text(
      ...
      aes(x = Tissue, y = 5, label = "Assay Not run."),
      inherit.aes = FALSE,
      size = 2.5,
      angle = 90,
      vjust = -0.3
...
make_a_plot_for_tissue_heatmaps = function(select_tissue_types){
  combined_ome_metadata = list(); current_ome = "TRNSCRPT"
  ...
  for (tissue_value in select_tissue_types){
    ...
    da_res_tissue = da_res_check %>%
      ...
      ungroup() %>%
      select(gene_symbol, assay, tissue, group, zscore, sex) %>%
      mutate(assay = "TRNSCRPT")

    ome_metadata_zscore = MotrpacRatTraining6moData::PHENO %>%
      select(sex, group, tissue) %>%
      distinct() %>%
      right_join(da_res_tissue, by = c("sex", "group", "tissue")) %>%
      mutate(group = factor(group, levels = c("1w", "2w", "4w", "8w"))) %>%
      arrange(sex, group) %>%
      distinct(sex, group, assay, tissue, gene_symbol, .keep_all = TRUE) %>%
      rename(value = zscore) %>%
      group_by(sex, group, assay, tissue) %>%
      pivot_wider(id_cols = c(sex, group, tissue),
                  values_from = value,
  ...
  annotation_df = annotation_df %>%
    mutate(Sex = sex,
           Timepoint = group,
           Tissue = tissue)  # Keep these for plotting
  ...
  ht <- ComplexHeatmap::Heatmap(
    ...
    cell_fun = function(j, i, x, y, width, height, fill) {
      if(!is.na(final_ome_metadata[i, j]) && abs(final_ome_metadata[i, j]) > 1.96) {
        grid::grid.text("*", x = x, y = y, gp = grid::gpar(fontsize = 6 * scale))
      }
    }
  )
...
make_immuno_heatmap = function(desired_feature){
  ...
  immuno_pvalue_annotations = MotrpacRatTraining6mo::combine_da_results(tissues = immuno_tissue_types,
                                                                        ...
                                                                        include_epigen = F) %>%
    filter(feature_ID == desired_feature) %>%
    select(tissue, sex, comparison_group, p_value) %>%
    tidyr::pivot_wider(id_cols = tissue,
                       names_from = c(sex, comparison_group),
                       values_from = p_value) %>%
    column_to_rownames("tissue") %>%
    as.data.frame()
  annotation_matrix <- ifelse(immuno_pvalue_annotations < p_value_cutoff, "*", "")
